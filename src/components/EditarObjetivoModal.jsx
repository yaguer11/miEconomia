...
export default function EditarObjetivoModal({ objetivo, onSave, onClose }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    setError('')
    if (!form.nombre.trim()) { setError('Ingresá un nombre para el objetivo'); return }
    setLoading(true)
    try {
      await onSave({
        ...form,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="glass rounded-2xl p-6 w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-bold text-white">Editar Objetivo</h2>
          <button id="close-editar-objetivo-modal" onClick={onClose} className="p-2 rounded-xl text-slate-400 hover:text-white hover:bg-white/5 transition-colors">
            <X size={18} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-3">
            <label className="text-sm font-medium text-slate-300">Emoji representativo</label>
          ...
          {/* Nombre */}
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">Nombre del objetivo</label>
            <input id="objetivo-nombre" type="text" value={form.nombre} onChange={e => setForm({ ...form, nombre: e.target.value })}
          ...
          <div className="space-y-2">
            <label className="block text-sm font-medium text-slate-300">Meta de ahorro <span className="text-slate-500">(opcional)</span></label>
            <input id="objetivo-meta" type="number" min="0" value={form.meta} onChange={e => setForm({ ...form, meta: e.target.value })}
              placeholder="Ej: 5000"
              className="w-full px-4 py-3 rounded-xl bg-slate-800/60 border border-slate-700/50 text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm transition-all" />
            <CurrencyToggle
              value={form.moneda_meta}
              onChange={(m) => setForm({ ...form, moneda_meta: m })}
              idPrefix="objetivo-meta-moneda"
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button id="save-objetivo" type="submit" disabled={loading}
